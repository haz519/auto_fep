...
class CGUIBrowserProcess(Process):
    ...
    def copy_dir(self, ncopy, signal=True):
        ...
        if signal:
            self.done_q.put(('CONTINUE', self.fep_case))
    ...
    def go_next(self, fep_text=None):
        self.browser.find_by_id('nextBtn').click()
        if fep_text:
            self.wait_text_multi([fep_text, self.CHARMM_ERROR])
    ...
    def run(self):
        with Browser(self.browser_type) as browser:
            ...
            for fep_case in iter(self.todo_q.get, 'STOP'):
                try:
                    self.fep_case = fep_case
                    print(self.name, "starting", fep_case['label'])
                    start_time = time.time()
                    resume_link = 0
                    base = os.path.abspath(fep_case['base'])
                    self.base = base
                    if 'jobid' in fep_case:
                        jobid = fep_case['jobid']
                        resume_link = fep_case['resume_link']
                        self.resume_step(jobid, link_no=resume_link)
                    else:
                        self.init_system(fep_case)

                    jobid = fep_case['jobid']
                    print(self.name, "Job ID:", jobid)

                    steps = fep_case['steps'][resume_link:]
                    failure = False
                    ...
                    # early failure?
                    if failure:
                        self.done_q.put(('FAILURE', fep_case, step_num, elapsed_time))
                        failure = False
                        continue

                    # late failure?
                    found_text = self.wait_text_multi([fep_case['final_wait_text'], self.CHARMM_ERROR])
                    if found_text == self.CHARMM_ERROR:
                        self.done_q.put(('FAILURE', fep_case, step_num, elapsed_time))
                        failure = False
                    else:
                        self.done_q.put(('SUCCESS', fep_case, elapsed_time))
                except Exception as e:
                    import sys, traceback
                    # give the full exception string
                    exc_str = ''.join(traceback.format_exception(*sys.exc_info()))
                    self.done_q.put(('EXCEPTION', fep_case, step_num, exc_str))
                    if self.pause:
                        print(self.name, "pausing; interrupt to exit")
                        while True:
                            time.sleep(1)
